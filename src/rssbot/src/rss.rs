...
const REALM: &'static str = "NEWS";
const RSS_FEED: &'static str = "https://feeds.bbci.co.uk/news/world/rss.xml";
...
pub async fn go() -> Result<(), String> {
    let url = Url::parse(RSS_FEED).map_err(|err| format!("url parsing failed: {:?}", err))?;
    ...
    let last_timestamp = read(|s| s.last_entry_timestamp);
    ...
    {
        mutate(|state| {
            schedule_message(state, message, Some(REALM.into()));
